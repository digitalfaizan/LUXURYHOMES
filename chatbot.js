  ...
  try {
    ...
    setTimeout(() => {
      matches.forEach((prop, idx) => {
        setTimeout(() => {
          try { addPropertyCard(prop); } catch(e) { console.warn('Card render error:', e); }
        }, idx * 300);
      });
    ...
    console.error('showMatchedProperties failed:', err);
    addBotMsg("Let me connect you with our expert directly for the best options! 😊");
    setTimeout(() => goToStep('ask_name'), 500);
  }
}
...
function addPropertyCard(prop) {
  ...
  card.innerHTML = `
    <div class="lh-prop-card-head">
      <span class="ptitle">${prop.emoji} ${escHtml(prop.title)}</span>
      <span class="pprice">${escHtml(prop.price)}</span>
    </div>
    <div class="lh-prop-card-body">
      <div class="lh-prop-loc">📍 ${escHtml(prop.area)}</div>
      <div class="lh-prop-tags">
        ${prop.size ? `<span>📐 ${escHtml(prop.size)}</span>` : ''}
        ${prop.bhk  ? `<span>🛏 ${prop.bhk} BHK</span>` : ''}
        ${prop.tags.map(t => `<span>${escHtml(t)}</span>`).join('')}
      </div>
      <button class="lh-prop-interested" type="button">
        👍 I'm Interested
      </button>
    </div>`;

  // Bind click handler safely via addEventListener — no string interpolation risk
  const btn = card.querySelector('.lh-prop-interested');
  btn.addEventListener('click', () => selectProperty(prop.id, prop.title, prop.price, btn));
